...
public class AdvTest3Activity extends AppCompatActivity {

    private String[] questions = {
            "1. If the second derivative f''(x) > 0, the function is called what?",
            "2. What inequality relates the value of a function at an average to the average of the function's values?",
            "3. If f is convex, is f((x+y)/2) <= or >= (f(x)+f(y))/2? (Type <= or >=)",
            "4. What inequality is formulated as x^t(x-y)(x-z) + ... >= 0?",
            "5. Does Schur's Inequality apply to negative real numbers? (yes/no)",
            "6. In Schur's proof, we assume WLOG that x >= y >= ? (Type the variable)",
            "7. If t=1 in Schur's inequality, what is the coefficient of xyz after expansion?",
            "8. What is the process of comparing sequence (3,0,0) and (1,1,1) called?",
            "9. Does the sequence (3,0,0) majorize (1,1,1)? (yes/no)",
            "10. Who formulated the theorem regarding majorization of symmetric sums?",
            "11. For Jensen's inequality, the sum of weights (w1+w2+...+wn) must equal what?",
            "12. Is the function f(x) = -ln(x) convex or concave for x>0?",
            "13. If A majorizes B, Muirhead states that T[A] is >= to what? (Type T[B])",
            "14. Can Muirhead's inequality be applied to functions that are not polynomials? (yes/no)",
            "15. Does the sequence (2,1,0) majorize (1,1,1)? (yes/no)",
            "16. In Schur's inequality, equality holds if x=y=z, or two are equal and the third is what?",
            "17. Is Schur's inequality symmetric with respect to all variables? (yes/no)",
            "18. What function is often used with Jensen's to prove AM-GM? (Type ln or log)",
            "19. What is the degree of the expanded polynomial in Schur's inequality when t=1?",
            "20. Can Jensen's inequality be used to find maximum areas of polygons? (yes/no)"
    };

    private String[] correctAnswers = {
            "convex", "jensen", "<=", "schur", "no", "z", "3", "majorization", "yes", "muirhead",
            "1", "convex", "t[b]", "no", "yes", "0", "yes", "ln", "3", "yes"
    };
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_adv_test3);
        ...
        findViewById(R.id.btnNextLesson).setOnClickListener(v -> {
            // Intent to AdvLesson4Activity if you have it created
    ...
    private void showFinalResult(int score) {
        ...
        if (score < 15) {
            tvFeedback.setText("This was extremely difficult. Re-read Muirhead and Schur carefully.");
            ...
            saveLessonStars(43, 0); // ID 43 = Advanced 3
            try { MediaPlayer.create(this, R.raw.sad).start(); } catch(Exception e){}
        } else {
            tvFeedback.setText("Brilliant! You've conquered the hardest inequalities.");
            ...
            saveLessonStars(43, earnedStars); // ID 43 = Advanced 3

            SharedPreferences prefs = getSharedPreferences("MyPrefs", MODE_PRIVATE);
            prefs.edit().putBoolean("adv_lesson4_unlocked", true).apply();
            prefs.edit().putInt("adv_test3_score", score).apply();
        }
    }
