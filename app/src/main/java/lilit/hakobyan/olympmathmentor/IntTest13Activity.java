...
public class IntTest13Activity extends AppCompatActivity {

    private String[] questions = {
            "1. Ceva's theorem proves that three lines in a triangle intersect at one point. What is the product of the side ratios?",
            "2. Which theorem is used to prove that three points lie on a single straight line? (Type the name)",
            "3. What is the name of a line segment connecting a vertex of a triangle to the opposite side?",
            "4. In the Euler line, the ratio of the distance HG (Orthocenter to Centroid) to GO (Centroid to Circumcenter) is what?",
            "5. Which major triangle center is NOT guaranteed to be on the Euler line? (Type the name)",
            "6. How many distinct points lie on the Nine-Point Circle of a triangle?",
            "7. The radius of the Nine-Point Circle is exactly what fraction of the Circumcircle's radius? (Type: half, third, or quarter)",
            "8. Does Menelaus's theorem require the intersecting line to cut through all three actual sides of the triangle? (Yes or No)",
            "9. In an equilateral triangle, the Orthocenter, Centroid, and Circumcenter are all the same point. (Yes or No)",
            "10. Ceva's theorem: (AF/FB) * (BD/DC) * (CE/EA) = ? (Type the number)",
            "11. Menelaus's theorem involves ratios that multiply to -1 (magnitude 1). Points that lie on the same line are called what?",
            "12. The center of the Nine-Point Circle is exactly the midpoint between the Circumcenter and the ____.",
            "13. If HG = 8, what is the distance from the Centroid (G) to the Circumcenter (O)?",
            "14. If the radius of the circumcircle is 10, what is the radius of the Nine-Point Circle?",
            "15. A transformation that scales an object from a center point is called ____ (Starts with 'H')."
    };
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_int_test13);
        ...
        findViewById(R.id.btnNextLesson).setOnClickListener(v -> {
            Intent intent = new Intent(IntTest13Activity.this, IntLesson14Activity.class);
    ...
    private void checkResults() {
        ...
        for (int i = 0; i < questions.length; i++) {
            ...
            } else if (userAnswer.equals(correctAnswer) || (i==14 && userAnswer.contains("homo"))) {
                score++;
    ...
    private void showFinalResult(int score) {
        ...
        if (score < 10) {
            tvFeedback.setText("These are complex theorems. Review Ceva, Menelaus, and the Euler line, then try again.");
            ...
            tvFeedback.setText("Congratulations! You are officially an Olympiad Geometry Master.");
            ...
            // Intermediate Lesson 13 identifier is 33
            saveLessonStars(33, earnedStars);

            SharedPreferences prefs = getSharedPreferences("MyPrefs", MODE_PRIVATE);
            prefs.edit().putBoolean("int_lesson14_unlocked", true).apply();
            prefs.edit().putInt("int_test13_score", score).apply();
        }
    }
