...
public class IntTest17Activity extends AppCompatActivity {

    private String[] questions = {
            "1. The first step of mathematical induction is called the ____ Case.",
            "2. The assumption that P(k) is true is called the Inductive ____.",
            "3. Induction is often compared to a line of falling ____.",
            "4. The sum of the first 'n' odd natural numbers equals what? (Use 'n')",
            "5. The formula for the sum of the first n natural numbers is n(n+1) / ? (Type the number)",
            "6. A variation where we assume the statement is true for ALL steps from 1 to k is called ____ Induction.",
            "7. The sum of the squares: 1^2 + 2^2 + ... + n^2 = n(n+1)(2n+1) / ? (Type the number)",
            "8. Is mathematical induction used to discover new formulas, or just to prove them? (Type: prove or discover)",
            "9. If P(k) implies P(k+1), and P(5) is true, is P(4) definitely true? (yes or no)",
            "10. Mathematical induction applies to statements depending on a ____ number n. (Type: natural or real)",
            "11. To prove that 3^(2n) - 1 is divisible by 8, in the base case (n=1), the expression equals what?",
            "12. Every integer > 1 can be written as a product of primes. This is proven using ____ induction. (Type: strong or weak)",
            "13. In the inductive step, you assume P(k) is true, and then prove it for what? (Type algebraically)",
            "14. The inequality 2^n >= n^2 is true for all natural numbers n >= ? (Type the smallest such number)",
            "15. In proving divisibility by induction, you often substitute the inductive hypothesis by representing it as a multiple, like 3M. (Type: true or false)"
    };
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_int_test17);
        ...
        findViewById(R.id.btnNextLesson).setOnClickListener(v -> {
            Intent intent = new Intent(IntTest17Activity.this, IntLesson18Activity.class);
    ...
    private void checkResults() {
        ...
        for (int i = 0; i < questions.length; i++) {
            ...
            // Handling acceptable variation for n^2
            if (i == 3 && (userAnswer.equals("n squared") || userAnswer.equals("n*n") || userAnswer.equals("n^2"))) {
                userAnswer = "n^2";
            }
    ...
    private void showFinalResult(int score) {
        ...
        if (score < 10) {
            tvFeedback.setText("Proofs can be tough! Review the Domino principle and try again.");
            ...
            tvFeedback.setText("Brilliant! You've mastered Mathematical Induction.");
            ...
            // Intermediate Lesson 17 identifier is 37
            saveLessonStars(37, earnedStars);

            SharedPreferences prefs = getSharedPreferences("MyPrefs", MODE_PRIVATE);
            prefs.edit().putBoolean("int_lesson18_unlocked", true).apply();
            prefs.edit().putInt("int_test17_score", score).apply();
        }
    }
